...
% syms theta
% r=h*k0*n1.*cos(theta);
% s0=-2.*atan((sqrt((n1^2)*(k0^2) -(k0*n1*sin(theta)).^2))./(sqrt((k0*n1*sin(theta)).^2 -(n2^2)*(k0^2))));
% s1=pi -2.*atan((sqrt((n1^2)*(k0^2) -(k0*n1*sin(theta)).^2))./(sqrt((k0*n1*sin(theta)).^2 -(n2^2)*(k0^2))));
% s2=2*pi -2.*atan((sqrt((n1^2)*(k0^2) -(k0*n1*sin(theta)).^2))./(sqrt((k0*n1*sin(theta)).^2 -(n2^2)*(k0^2))));
% s3=3*pi -2.*atan((sqrt((n1^2)*(k0^2) -(k0*n1*sin(theta)).^2))./(sqrt((k0*n1*sin(theta)).^2 -(n2^2)*(k0^2))));

% Método de Newton para hallar las soluciones (intersecciones de las gráficas)
% f = r-s1;
% df = diff(f)
% f=@(theta) h*k0*n1.*cos(theta) - 4*pi +2.*atan((sqrt((n1^2)*(k0^2) -(k0*n1*sin(theta)).^2))./(sqrt((k0*n1*sin(theta)).^2 -(n2^2)*(k0^2))));
% df=@(theta) (2*((9*pi^2*cos(theta)*sin(theta))/((9*pi^2*sin(theta)^2 - 2778046668940015/70368744177664)^(1/2)*(3125302502557517/35184372088832 - 9*pi^2*sin(theta)^2)^(1/2)) + (9*pi^2*cos(theta)*sin(theta)*(3125302502557517/35184372088832 - 9*pi^2*sin(theta)^2)^(1/2))/(9*pi^2*sin(theta)^2 - 2778046668940015/70368744177664)^(3/2)))/((9*pi^2*sin(theta)^2 - 3125302502557517/35184372088832)/(9*pi^2*sin(theta)^2 - 2778046668940015/70368744177664) - 1) - 3*pi*sin(theta);
% p0= 1;
% delta=1e-6;
% epsilon=1e-6;
% max=20;
